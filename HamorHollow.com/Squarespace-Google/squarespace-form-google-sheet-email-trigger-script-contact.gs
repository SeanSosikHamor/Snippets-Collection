...
function sendEmails() {
  ...
  for (var row = 1; row < values.length; row++) {
    if (values[row][0] && !values[row][6])  {         // If date ([row][0] Cell A) has a value and row not previously sent ([row][6] Cell G) then send email.
      ...
      s.getRange(row + 1, 7).setValue('Sent');        // Sets Row Cell G (values[row][6]) to "Sent" once email is sent. This is an intentional off-by-one. Setting 7 here actually sets values[row][6].
    }
  }
}
